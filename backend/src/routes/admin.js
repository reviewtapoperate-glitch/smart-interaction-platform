...
function adminRequired(req, res, next) {
  ...
  if (!env.PLATFORM_ADMIN_KEY || !key || key !== env.PLATFORM_ADMIN_KEY) return res.status(404).json({ error: "Not found" });
  next();
}
...
router.get("/onboarding", asyncRoute(async (_req, res) => {
  const rows = await prisma.$queryRawUnsafe(`SELECT o.*, b.name AS "businessName", b."createdAt" AS "businessCreatedAt" FROM "OnboardingRequest" o JOIN "Business" b ON b.id=o."businessId" ORDER BY o."updatedAt" DESC`);
  res.json(rows);
}));
...
router.get("/nfc", asyncRoute(async (_req, res) => {
  const rows = await prisma.$queryRawUnsafe(`SELECT n.*, b.name AS "businessName", e.name AS "endpointName", e."publicToken" FROM "NfcTag" n LEFT JOIN "Business" b ON b.id=n."businessId" LEFT JOIN "Endpoint" e ON e.id=n."endpointId" ORDER BY n."createdAt" DESC`);
  res.json(rows);
}));
...
router.post("/nfc", asyncRoute(async (req, res) => {
  ...
  const result = await prisma.$queryRawUnsafe(`INSERT INTO "NfcTag" ("id","inventoryCode","businessId","endpointId","writeUrl") VALUES ($1,$2,$3,$4,$5) RETURNING *`, randomUUID(), data.inventoryCode, data.businessId || null, data.endpointId || null, writeUrl);
  res.status(201).json(result[0]);
}));
