...
const endpointActions = [
  "REVIEW_CLICKED", "WHATSAPP_CLICKED", "WEBSITE_CLICKED", "SOCIAL_CLICKED", "DIRECTIONS_CLICKED",
  "SAVE_CONTACT_CLICKED", "SHARE_CLICKED", "PAYMENT_COPIED", "MENU_OPENED", "GALLERY_OPENED"
];
...
router.get("/endpoint/:token", asyncRoute(async (req, res) => {
  ...
  res.json({
    ...
    business: {
      ...
      timezone: endpoint.business.timezone,
      websiteUrl: endpoint.business.websiteUrl,
      googleBusinessUrl: endpoint.business.googleBusinessUrl,
...
router.get("/session/:token", asyncRoute(async (req, res) => {
  ...
  res.json({
    session: { id: session.id, token: session.publicToken, status: session.status, endpoint: session.endpoint.name },
...
router.get("/session/:token/products", asyncRoute(async (req, res) => {
  ...
  if (!session) return res.status(404).json({ error: "Session not found" });
  res.json(await prisma.product.findMany({ where: { businessId: session.businessId, active: true }, orderBy: { name: "asc" } }));
}));
...
router.post("/session/:token/orders", asyncRoute(async (req, res) => {
  ...
  const data = z.object({
    items: z.array(z.object({ productId: z.string(), quantity: z.number().int().positive().max(100) })).min(1),
    notes: z.string().max(500).optional()
  }).parse(req.body);
  const products = await prisma.product.findMany({ where: { businessId: session.businessId, id: { in: data.items.map(i => i.productId) }, active: true } });
  ...
  const order = await prisma.order.create({
    data: {
      ...
      notes: data.notes,
      status: "SUBMITTED",
      items: { create: data.items.map(item => { const product = byId.get(item.productId); return { productId: product.id, quantity: item.quantity, unitPriceMinor: product.priceMinor, nameSnapshot: product.name }; }) }
    },
    include: { items: true }
  });
  await recordEvent({ businessId: session.businessId, endpointId: session.endpointId, sessionId: session.id, eventName: "ORDER_CREATED", metadata: { orderId: order.id } });
