...
app.get(`${API_ROOT}`, (_req, res) => res.json({ service: "ReviewTap", version: "1.3.0", status: "ready" }));
app.get("/health", (_req, res) => res.status(200).json({ status: "ok", service: "ReviewTap" }));
...
io.on("connection", socket => {
  socket.on("session:join", ({ sessionId }) => { if (typeof sessionId === "string" && sessionId) socket.join(`session:${sessionId}`); });
  socket.on("endpoint:join", ({ endpointId }) => { if (typeof endpointId === "string" && endpointId) socket.join(`endpoint:${endpointId}`); });
});
...
app.use((err, _req, res, _next) => {
  ...
  const details = isValidation && Array.isArray(err.issues) ? err.issues.map(issue => `${issue.path?.length ? issue.path.join(".") : "request"}: ${issue.message}`).join("; ") : null;
  const message = isValidation ? `Invalid request data${details ? ` — ${details}` : ""}` : (env.NODE_ENV === "production" && status === 500 ? "Internal server error" : err.message);
...
async function start() {
  ...
  const server = httpServer.listen(env.PORT, "0.0.0.0", () => console.log(`Backend running on port ${env.PORT}`));
  async function shutdown(signal) { console.log(`Received ${signal}; shutting down`); server.close(async () => { await prisma.$disconnect(); process.exit(0); }); }
